...
class _Discriminator(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.main = nn.Sequential(
            ...
            nn.Dropout2d(p=0.5),
            Weight_norm(
                nn.Conv2d(96, 192, 3, 1, 1, bias=False), name="weight"
            ),  # 16*16
            nn.LeakyReLU(0.2),
            Weight_norm(
                nn.Conv2d(192, 192, 3, 1, 1, bias=False), name="weight"
            ),  # 16*16
            nn.LeakyReLU(0.2),
...
class Train(object):
    ...
    def log_sum_exp(self, x):
        ...
        return torch.log(torch.sum(torch.exp(x - m.unsqueeze(1)), dim=1)) + m
    ...
    def discriminator_training(self, x, y, x_unlabel):
        ...
        # loss
        loss_lab = torch.mean(torch.mean(self.log_sum_exp(lab))) 
                   - torch.mean(torch.gather(lab, 1, y.unsqueeze(1)))
        loss_unlab = 0.5 * (
            torch.mean(F.softplus(self.log_sum_exp(unlab)))
            - torch.mean(self.log_sum_exp(unlab))
            + torch.mean(F.softplus(self.log_sum_exp(gen_output)))
        )
